...
public class ComponentVoidProductionRoom extends ComponentPyramidRoom {
        ...
        public void replaceAltWall(World world, StructureBoundingBox sbb, int x, int y, int z) {
            Block b = getBlockAtCurrentPosition(world, x, y, z, sbb);
            int m = getMetaAtCurrentPosition(world, x, y, z, sbb);
            boolean shouldReplace = (b == PyramidMain.headBlockID && m == PyramidMain.headBlockMeta) || (b == PyramidMain.rootBlockID && m == PyramidMain.rootBlockMeta);
            if (shouldReplace) {
        ...
        public boolean addComponentParts(World world, Random rand, StructureBoundingBox sbb) {
            for (int y = 1; y < 2*roomHeight; ++y) {
                for (int x = 0; x <= roomWidth; x += roomWidth) {
                    for (int z = 0; z <= roomDepth; ++z) {
                        replaceAltWall(world, sbb, x, y, z);
                    }
                }
                for (int x = 0; x <= roomWidth; ++x) {
                    for (int z = 0; z <= roomDepth; z += roomDepth) {
                        replaceAltWall(world, sbb, x, y, z);
                    }
                }
            }
