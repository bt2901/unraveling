package unraveling.entity.boss;
...
public class EntityTFLich extends EntityMob implements IBossDisplayData {
	...
	public EntityTFLich(World world) {
		super(world);
		setSize(1.1F, 2.5F);
        //texture = UnravelingMod.MODEL_DIR + "twilightlich64.png";
//        this.yOffset = .25F;
    ...
    {
        if (isShadowClone()){
            return cloneHeldItems[getPhase() - 1];
        }
        return masterHeldItems[getPhase() - 1];
    }
	...
	private void dropGoldThing() {
		ItemStack goldThing;
		int thingType = rand.nextInt(2);
		if (thingType == 0) {
			// goldThing = new ItemStack(ConfigItems.itemWandRond, 1, 7);
			// goldThing = new ItemStack(Items.ender_pearl);
			// goldThing = ItemApi.getItem("WandRond", 7);
            String desc = "itemBowBone";
            goldThing = ItemApi.getItem(desc, 0);
		}
		else if (thingType == 1) {
			// goldThing = ItemApi.getItem("itemWandRond", 7);
            String desc = "itemWandRod";
            goldThing = ItemApi.getItem(desc, 7);
		}
		else if (thingType == 2) {
			//goldThing = ItemApi.getItem("itemWandRod", 57);
			goldThing = new ItemStack(Items.ender_pearl);
			// goldThing = new ItemStack(ConfigItems.itemWandRond, 1, 57);
		} else {
			goldThing = new ItemStack(Items.ender_pearl);
        }
        // also: bone (biome o plenty), bone lever, skeletal bees, bone combs
        // bone staff core / bone wand capped with something
        /*
		else if (thingType == 3) {
	...
	public int getPhase() {
        ...
        int maxHealth = (int)this.getMaxHealth();
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, float damage) {
		...
		System.out.println("Can pierce: " + (par1DamageSource.isUnblockable() && damage > 2));
		System.out.println("Damage: " + damage);
		System.out.println("Unblockable: " + par1DamageSource.isUnblockable());
		...
		if (getShieldStrength() > 0)
		{
            boolean willPierce = false;
            // willPierce = (par1DamageSource.getDamageType() == "explosion" && par1DamageSource.getSourceOfDamage() == null) || par1DamageSource.isUnblockable();
            willPierce = (par1DamageSource.getDamageType() == "explosion" && par1DamageSource.getSourceOfDamage() == null && damage >= 15);
            willPierce = true;
			if (willPierce) 
			{
				...
				if (getShieldStrength() > 0) {
                    ...
                    String sound_string = UnravelingMod.ID + ":mob.lich.break";
					this.worldObj.playSoundAtEntity(this, sound_string, 1.0F, ((this.rand.nextFloat() - this.rand.nextFloat()) * 0.7F + 1.0F) * 2.0F);
				}
			}
			else
			{
                String sound_string = UnravelingMod.ID + ":mob.lich.block";
				this.worldObj.playSoundAtEntity(this, sound_string, 1.0F, ((this.rand.nextFloat() - this.rand.nextFloat()) * 0.7F + 1.0F) * 2.0F);
        ...
        // dramatic dying animation
        //if (damage > this.getHealth() && isDying() == 0) {
        //    damage = this.getHealth() - 1;
...
//	}
    ...
    protected void attackEntity(Entity targetedEntity, float f)
    {
    	...
    	if (getPhase() == 1) {
    		...
    		if (attackTime == 0) {
	    		...
	    		if (f < 2.0F) {
                    ...
                    // else cast something
                    attackAsUsual(targetedEntity, f, 60);
                }
    		}
    		// do not chase
    		hasAttacked = true;
    	...
    	if (getPhase() == 2 ) {
	    	if (attackTime % 15 == 0 && !worldObj.isRemote) {
	    		if (!isShadowClone()) {
	    	...
	    	if (attackTime == 0) {
                attackAsUsual(targetedEntity, f, 100);
            }
            ...
            if (!isShadowClone()) {
                if (attackTime == 0) {
                    ...
                    if (f < 2.0F) {
                        ...
                        // else cast something
                        attackAsUsual(targetedEntity, f, 50);
                    }
                }
            }
            if (isShadowClone()) {
                // melee!
                // shadow clone go melee
     ...
     */
	protected void attackAsUsual(Entity targetedEntity, float f, int nextAttackTime) {
        if (f < 20F && canEntityBeSeen(targetedEntity)) {
            // if we can see the target, launch a bolt
            if (this.getNextAttackType() == 0)
            {
...
//					worldObj.playSoundAtEntity(mob, mob.getDeathSound(), mob.getSoundVolume(), (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
	...
	protected boolean canPop(Entity nearby) {
		Class[] poppable = {EntitySkeleton.class, EntityZombie.class, EntityEnderman.class, EntitySpider.class, EntityCreeper.class};
    ...
    protected void checkAndSpawnClones(Entity targetedEntity) {
		System.out.println("Lich is checking for a shadow clone!");
    	// if not, spawn one!
	...
	protected void checkAndSpawnMinions(Entity targetedEntity) {
		System.out.println("Lich is checking for minions!" + (!worldObj.isRemote && this.getMinionsToSummon() > 0));
...
//        System.out.println("canEntitySee = " + canEntitySee(targetEntity, tx, ty, tz));
    ...
    {
        return UnravelingMod.ID + ":mob.lich.breathe";
    ...
    {
        return UnravelingMod.ID + ":mob.lich.hurt";
    }
    ...
    {
        return UnravelingMod.ID + ":mob.lich.death";
    }
